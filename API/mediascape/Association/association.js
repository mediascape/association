define( ["jquery","qrcode","webcodecam","qrcodelib","receiver","sender","presentation"], function($) {
	var Association = function() {
		...
		******************************************************************************************/
		...
		/*****************************************************************************************
		*
		*	mediascape.association.doAssociation('qr', "place", url, short[, width, height])
		*
		...
		******************************************************************************************/
				...
				var p1= new Promise(
					function(resolve,reject){
						...
						if(short&&url.indexOf("bit.ly")==-1&&bitlyUser!=""&&bitlyPass!=""){
							...
							return visualizeQr(placeId,url,width,height,resolve);
						}
				});
				return p1;
			}
			...
			function visualizeQr(placeId,url,width,height,resolve){
				var shortedUrl = url;
						...
						$.ajax({
							...
							success: function (json) {
								...
								if(url&&url.indexOf("bit.ly")==-1&&bitlyUser!=""&&bitlyPass!=""){
									...
									createMessageEvent("shakeChange", true, "<p>It has not been possible to minimize the url.</p><p> Define a Bitly user and pass.</p>", 1);
								}
							}
						});
					});
						...
						$.ajax({
							...
							success: function (json) {
								...
								if(url&&url.indexOf("bit.ly")==-1&&bitlyUser!=""&&bitlyPass!=""){
									...
									resolve(JSON.parse('{"response":"<p>It has not been possible to minimize the url.</p><p> Define a Bitly user and pass.</p>"}'));
								}
							}
						});
					});
