...
/**
 * MemoryDB.js
 * Quick n Dirty inMemory DB for MediaScape Shared State
 *
...
function MongoDB() {
    ...
    function init() {
        ...
        Object.keys(config.paths).forEach(function (path) {
            ...
            stateModels[path] = db.model(path, stateSchema);
	...
	var safe = {
        ...
        userModel = db.model(config.mongoPath + '_user', userSchema);
        appModel = db.model(config.mongoPath + '_app', appSchema);
        userAppModel = db.model(config.mongoPath + '_appUser', userAppSchema);
        groupModel = db.model(config.mongoPath + '_group', groupSchema);
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                (function (cElement) {
                    stateModels[path].findOne({
                        ...
                        if (!err && !doc && cElement.type != 'remove') {
                            var doc = new stateModels[path]();
                            doc.key = cElement.key;
                            doc.value = cElement.value;
                            doc.save();
                        } else if (doc) {
                            if (cElement.type == 'set') {

                                if (JSON.stringify(doc.value) != JSON.stringify(cElement.value) && JSON.stringify(cElement.key) == JSON.stringify(doc.key)) {
                                    doc.value = cElement.value;
                                    doc.save();
                                }
                                ...
                                if (JSON.stringify(cElement.key) == JSON.stringify(doc.key)) {
                                    doc.value = cElement.value;
                                    doc.remove();
                                }
                            }
    ...
    function getUserMappings(request, callback) {
